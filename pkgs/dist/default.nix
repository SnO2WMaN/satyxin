  ...
  {pkgs, ...}:
    pkgs.stdenv.mkDerivation {
      ...
      installPhase = ''
        ...
        mkdir -p $out/lib/satysfi/{fonts,hash,hyph,,packages,unidata}

        cp -r $distsatysfi/hash/* $out/lib/satysfi/hash
        cp -r $distsatysfi/hyph/* $out/lib/satysfi/hyph
        cp -r $distsatysfi/packages/* $out/lib/satysfi/packages
        cp -r $distsatysfi/unidata/* $out/lib/satysfi/unidata

        for file in $(find $(echo $src | tr ' ' '\n' | grep -E ".*-lm2004otf") -name "*.otf"); do
            cp $file $out/lib/satysfi/fonts
        done
